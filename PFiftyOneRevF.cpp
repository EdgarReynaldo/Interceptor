...
// FindRoots provides a complex double interface to P51. It is only needed in the Iowa Hills IIR FIR Filter Code Kit.
/**
int FindRoots(int N, double *Coeff, CplxD *Roots)
{
 ...
 for(j=0; j<N; j++)Roots[j] = CplxD((double)RealRoot[j], (double)ImagRoot[j]); // long double to complex double
 return(N);
}
*/
//---------------------------------------------------------------------------
...
int PFiftyOne(long double *Coeff, int Degree, long double *RealRoot, long double *ImagRoot)
{
 ...
 TypeOfK = 0;
 while(N > P51_MINDEGREE && TypeOfK < MAX_NUM_K )
  {
  ...
  }  // The outermost loop while(N > P51_MINDEGREE)



 // Done, except for the last 1 or 2 or 3 or 4 roots, depending on the value used for P51_MINDEGREE
 // If N > P51_MINDEGREE at this point, we failed.
 if(N > P51_MINDEGREE)
  {
   Degree = N = 0;
 ...
 else if(N == 3 && P51_MINDEGREE >= 3)
  {
   j = Degree - N;
   CubicRoots(P, &RealRoot[j], &ImagRoot[j]);
  }

 else if(N == 4 && P51_MINDEGREE == 4)
  {
   j = Degree - N;
